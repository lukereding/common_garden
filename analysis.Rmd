...
generate_label_df <- function(x, y, dataframe, label_height){
  require(lmPerm)
  ...
  ## get the max value for each group
  ## need to revise to go something like this 
  boxplot.df <- dataframe %>%
    group_by(eval(arguments$x, dataframe)) %>%
    summarise(height = fivenum(eval(arguments$y, dataframe))[5])
  names(boxplot.df)[1] <- "plot.labels"
  # in this function
  
  # boxplot.df <- data.frame(plot.labels,
  #                          height = label_height)
  print(boxplot.df)
...
df_avg %>%
  ...
  ggtitle("average number of large -> large agg. per video") +
  theme_clean()
```
...
df_avg %$% 
  lmp(pairewise_distance_juvs ~ treatment, data = .) %>% 
  glht(linfct=mcp(treatment="Tukey")) %>% 
  summary
...
df %>%
  ...
  df_avg %$% 
  lmp(pairwise_distance_females ~ treatment, data = .) %>% 
  glht(linfct=mcp(treatment="Tukey")) %>% 
  summary

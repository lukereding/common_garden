...
```{r}
files <- list.files("/Users/lreding/Documents/common_garden/data/", pattern = "*.json", full.names = T)
...
df %<>% 
  rowwise %>%
  mutate(total_courtship =sum(large_courting, small_courting ,intermediate_courting, na.rm=TRUE))
...
write_csv(df_avg, "~/Documents/common_garden/df.csv")
```
...
# as bar plot
df$treatment <- factor(df$treatment, levels = c("LS", "INT", "SS", "LL","FF"))
require(ggthemes)
courts <- df %>%
  filter(good_video == TRUE) %>%
...
limits <- aes(ymax = avg_court + se_court, ymin=avg_court - se_court)
  courts <- df_avg %>%
    ...
    theme_mod() +
    # ylim(c(0,2)) +
    scale_x_discrete(breaks=c("LS","INT", "LL","SS", "FF"),
          labels=c("LS", "II", "LL", "SS", "FF")) +
    ggtitle("amount of courtship") +
    geom_errorbar(limits, position=position_dodge(width=0.9), width=0.1)+
    theme(axis.ticks.x = element_blank()) +
    theme(axis.text.x = element_blank()) +
  ...
  ggsave("~/Desktop/cg_fig_4_mary.pdf", height = 6, width = 3)
```
...
# Looking at foraging and activity levels

First step is to read in the JSON files from the computer where people score these videos:

```bash

# make new directory to contain the JSON files
mkdir ~/Desktop/json_files

scp lukereding@10.147.45.228:"/Volumes/THE\ MOLLUSK/common\ garden\ vids\ for\ foraging\ scoring/saved_videos_bkp/data/*.json" ~/Desktop/json_files
...
files <- list.files("~/Downloads/JSON_files", pattern = "*.json", full.names = T)
...
n <- length(files)
# create data frame
df <- data.frame("number_foraging_fish" = integer(n),
                   "number_foraging_events" =integer(n),
                   "number_active"= integer(n),
                   "tank_id"= integer(n),
                   "date"= character(n),
                   "comments"= character(n),
                   "video_name"= character(n),
                   "observer"= character(n),
                   "time_active"= character(n),
                 stringsAsFactors=FALSE)
...
for(i in 1:length(files)){
  ...
  # print(i)
  # extract the data
  df$number_foraging_fish[i] <-  ret(json_data$number_foraging_fish) %>% as.numeric
  df$number_foraging_events[i] <-  ret(json_data$number_foraging_events) %>% as.numeric
  df$number_active[i] <-  ret(json_data$number_active) %>% as.numeric
  df$tank_id[i] <-  ret(json_data$tank_id) %>% as.character
  df$date[i] <-  ret(json_data$date) %>% mdy %>% as.character
  df$comments[i] <-  ret(json_data$comments) %>% as.character
  df$video_name[i] <-  ret(json_data$video_name) %>% as.character
  df$observer[i] <-  ret(json_data$observer) %>% as.character
  df$time_active[i] <-  ret(json_data$time_active) %>% ms %>% period_to_seconds %>% as.numeric
}

# fix a typo
df$tank_id[df$tank_id == "LFF2"] = "FF2"
...
We have our data in a data frame now, but the data are still pretty 'raw'. We need to calculate some things to make these metrics slightly more interpretable. 
...
df_sum <- df %>%
  mutate(treatment = tank_id %>%  # make column for treatment
           str_sub(., 1, str_length(tank_id) - 1),
         treatment = factor(treatment, levels = c("LL", "LS", "INT", "SS", "FF"))) %>%
  mutate(time_active_per_fish = as.numeric(time_active) / number_active,
         foraging_per_fish = number_foraging_events / number_foraging_fish) %>% {
           write_csv(., "~/Documents/common_garden/foraging_activity.csv")
         } %>%
  group_by(treatment) %>%
  summarise(
    time_active_se = sd(as.numeric(time_active), na.rm = T) / n(), 
    time_active_sd = sd(as.numeric(time_active), na.rm = T), 
    time_active = mean(as.numeric(time_active), na.rm = T),
    number_foraging_fish_se = sd(number_foraging_fish, na.rm = T) / n(),
    number_foraging_fish_sd = sd(number_foraging_fish, na.rm = T),
    number_foraging_fish = mean(number_foraging_fish, na.rm = T),
    number_active_se = sd(number_active, na.rm = T) / n(),
    number_active_sd = sd(number_active, na.rm = T),
    number_active = mean(number_active, na.rm = T),
    time_active_per_fish_se = sd(time_active_per_fish, na.rm = T) / n(),
    time_active_per_fish_sd = sd(time_active_per_fish, na.rm = T),
    time_active_per_fish = mean(time_active_per_fish, na.rm = T),
    foraging_per_fish_sd = sd(foraging_per_fish, na.rm = T),
    foraging_per_fish_se = sd(foraging_per_fish, na.rm = T) / n(),
    foraging_per_fish = mean(foraging_per_fish, na.rm = T), 
    total_videos = n()
  )
...
Still need to normalize to 1 min, so since these videos are 10 seconds, I'll multiply everything by 6:

(Recall that the variance is based on _squared_ deviations from the mean, so we have to multiply the variance by 6^2 = 36 and not 6.)

```{r}

df_sum %<>%
  mutate(time_active_per_fish_per_minute = time_active_per_fish * 6,
         time_active_per_fish_per_minute_se = sqrt((time_active_per_fish_sd^2) * 36) / total_videos,
         foraging_per_fish_per_minute = foraging_per_fish * 6,
         foraging_per_fish_per_minute_se = sqrt((foraging_per_fish_sd^2) * 36) / total_videos)


# write it out
write_csv(df_sum, "~/Documents/common_garden/forging_activity_summarized.csv")
```

## plot the foraging / activity results
...
# time active by treatment
activity <- df_sum %>%
  ggplot(aes(x = treatment, y = time_active_per_fish)) +
  geom_col(aes(fill = treatment)) +
  geom_errorbar(aes(ymin=time_active_per_fish-time_active_per_fish_se, ymax=time_active_per_fish+time_active_per_fish_se), colour="black", width=0) +
  theme_pubr() +
  ylab("number of seconds per video per fish") +
  ggtitle("average time spent active\namong treatments") +
  scale_fill_world(guide = F)
  
# foraging events per treatment
foraging <- df_sum %>%
  ggplot(aes(x = treatment, y = foraging_per_fish)) +
  geom_col(aes(fill = treatment)) +
  geom_errorbar(aes(ymin=foraging_per_fish-foraging_per_fish_se, ymax=foraging_per_fish+foraging_per_fish_se), colour="black", width=0) +
  theme_pubr() +
  ylab("foraging events per video per fish") +
  ggtitle("average number of foraging events\namong treatments") +
  scale_fill_world(guide = F)

# plot em
cowplot::plot_grid(activity, foraging, labels = c("a", "b"), ncol = 1)
ggsave("~/Desktop/act_and_foraging.pdf", height = 8, width = 5)
```
...
To do the stats, we need the raw data, not that data in a summarized form. We'll re-read in the csv file we saved previously then do the stats tests:



```{r}

raw <- read_csv("~/Documents/common_garden/foraging_activity.csv") %>%
  mutate(time_active_per_fish_per_minute = time_active_per_fish * 6,
         time_foraging_per_fish_per_minute = foraging_per_fish * 6)
...
generate_label_df <- function(x, y, dataframe){
  ...
  arguments <- as.list(match.call())
  # do the anova
  model <- aov(eval(arguments$y, dataframe) ~ eval(arguments$x, dataframe), data = dataframe)
  print(anova(model))
...
activity <- raw %>%
  group_by(treatment) %>% 
  summarise(avg_activity = mean(time_active_per_fish_per_minute, na.rm = TRUE)) %>%
  ggplot(aes(x = treatment, y = avg_activity)) +
  geom_col(aes(fill = treatment)) +
  #geom_errorbar(aes(ymin=time_active_per_fish-time_active_per_fish_se, ymax=time_active_per_fish+time_active_per_fish_se), colour="black", width=0) +
  theme_pubr() +
  ylab("number of seconds per video per fish") +
  ggtitle("average time spent active\namong treatments") +
  scale_fill_world(guide = F)+
  geom_text(data = generate_label_df(x = treatment, y = time_active_per_fish_per_minute, raw), 
            aes(x = plot.labels, y = height, label = labels))

foraging <- raw %>%
  group_by(treatment) %>% 
  summarise(avg_foraging = mean(time_foraging_per_fish_per_minute, na.rm = TRUE)) %>%
  ggplot(aes(x = treatment, y = avg_foraging)) +
  geom_col(aes(fill = treatment)) +
  #geom_errorbar(aes(ymin=time_active_per_fish-time_active_per_fish_se, ymax=time_active_per_fish+time_active_per_fish_se), colour="black", width=0) +
  theme_pubr() +
  ylab("number of seconds per video per fish") +
  ggtitle("average time spent active\namong treatments") +
  scale_fill_world(guide = F)+
  geom_text(data = generate_label_df(x = treatment, y = time_foraging_per_fish_per_minute, raw),
            aes(x = plot.labels, y = height, label = labels)) +
  geom_text(data = raw %>% group_by(treatment) %>% tally, aes(x = treatment, y = 0.2, label = n))
